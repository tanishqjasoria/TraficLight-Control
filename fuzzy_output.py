...
def fuzzy(var_car,car_confi,var_truck,truck_confi,var_bike,bike_confi,obj):
    ...
    list=[81,94,64,69,96,99,86,85,84,99,51]
    R=list
    min_car=0
    i=0
    for i in range(0,len(list)):
    ...
    outpt_low=fuzz.trimf(x_congestion,[0 ,0 ,0.5])
    outpt_med=fuzz.trimf(x_congestion,[0.25 ,0.5 ,0.75])
    outpt_high=fuzz.trimf(x_congestion,[0.5 ,1 ,1])
    ...
    ax0.plot(x_congestion, outpt_low, 'g', linewidth=1.5, label='Low')
    ax0.plot(x_congestion, outpt_med, 'y', linewidth=1.5, label='Medium')
    ax0.plot(x_congestion, outpt_high, 'r', linewidth=1.5, label='High')
    ...
    ax1.plot(x_car, car_mf, 'b', linewidth=1.5, label='car')
    ax1.plot(x_car, car_low, 'g', linewidth=1.5, label='Low')
    ax1.plot(x_car, car_med, 'y', linewidth=1.5, label='Medium')
    ax1.plot(x_car, car_high, 'r', linewidth=1.5, label='High')
    ax1.set_title('car')
    ax1.legend()

    """ax2.plot(x_bike, bike_low, 'b', linewidth=1.5, label='Low')
    ax2.plot(x_bike, bike_med, 'g', linewidth=1.5, label='Medium')"""
    ax2.plot(x_bike, bike_mf, 'b', linewidth=1.5, label='bike')
    ax2.plot(x_bike, bike_low, 'g', linewidth=1.5, label='Low')
    ax2.plot(x_bike, bike_med, 'y', linewidth=1.5, label='Medium')
    ...
    ax3.plot(x_truck, truck_mf, 'b', linewidth=1.5, label='truck')
    ax3.plot(x_truck, truck_low, 'g', linewidth=1.5, label='Low')
    ax3.plot(x_truck, truck_med, 'y', linewidth=1.5, label='Medium')
    ax3.plot(x_truck, truck_high, 'r', linewidth=1.5, label='High')
    ax3.set_title('truck')
    ax3.legend()
    ...
    print "No of lanes: ", obj.num_lanes, " Camera Range: ", obj.cam_range, " Congestion: ", congestion, " when \nBike: ",S,"\nCar: ",R, "\nTruck: ",T
    plt.show()

Ge=sp.Path(100,1)
fuzzy(12,1,0,1,0,1,Ge)